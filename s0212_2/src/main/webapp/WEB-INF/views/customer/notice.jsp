					...
					<div class="orderDivMt">
						<table summary="NO, 제목, 등록일, 조회수 순으로 공지사항을 조회 하실수 있습니다." class="orderTable2" border="1" cellspacing="0">
							...
							<tbody>
								...
								<!-- 게시글 날짜 -->
								<fmt:formatDate value="${cdto.bdate}" var="day" pattern="yyyy-MM-dd"/>
								<c:if test="${day == today}">
									<tr>
										<td class="tnone">${cdto.bno}</td>
										<td class="left">
											<a href="/customer/nview?bno=${cdto.bno}">${cdto.btitle}</a>
								...
								<tr>
									<td class="tnone">${cdto.bno}</td>
									<td class="left">
										<a href="/customer/nview?bno=${cdto.bno}" class="lightgray">${cdto.btitle}</a>
									</td>
									<td>
										<fmt:formatDate value="${cdto.bdate}" pattern="yyyy-MM-dd"/>
									</td>
