				...
				<div id="mypage">
					...
					<div class="viewDivMt">
						<div class="viewHead">
							...
							<div class="day">
								<p class="txt">이벤트기간<span><fmt:formatDate value="${edto.stdate}" pattern="yyyy-MM-dd"/> ~ <fmt:formatDate value="${edto.enddate}" pattern="yyyy-MM-dd"/></span></p>
							</div>
						</div>
						...
						$(function(){
							...
							$(".replyBtn").click(function(){
								...
								$.ajax({
									...
									type:"post", // 보내는 타입
									data:{"eno":eno, "cpw":cpw, "ccontent":ccontent}, // 파라미터값
									dataType:"json", // 리턴받을 값의 형태
									success:function(data){ // data : cwrite의 리턴값
							...
							// 2. 댓글 삭제
							// $(".deleteBtn").click(function(){}); 정적 : 화면표시가 됐을때 추가된 html소스에는 적용이 안됨
							   $(document).on("click",".deleteBtn",function(){
								   cno = $(this).closest("ul").attr("id"); // deleteBtn에서 가장 가까운 ul의 id를 찾기
								   if(confirm(cno+"번 게시글을 삭제하시겠습니까?")){
										alert("삭제되었습니다.");
						   ...
						   // 3. 댓글 수정
							     ...
							     cno = $(this).closest("ul").attr("id");
						   		 ...
						   		 console.log($(this).closest("ul").attr("id")); // deleteBtn에서 가장 가까운 ul의 id를 찾기
						   		 ...
						   		 $("#"+cno).html(hdata) // html() 원래 있던 소스를 다 지우고 새로운 소스 넣기
						   	  }); // updateBtn : 댓글 수정
					   	  ...
					   	  // 4. 댓글 수정 취소
				   	  	 ...
				   	  	 // 5. 댓글 수정 저장
								...
								$.ajax({
									...
									type:"post", // 보내는 타입
									data:{"eno":eno, "cno":cno, "ccontent":ccontent}, // 파라미터값
									dataType:"json", // 리턴받을 값의 형태
									success:function(data){ // data : cwrite의 리턴값
										console.log(data.cno);
										console.log(data.ccontent);
